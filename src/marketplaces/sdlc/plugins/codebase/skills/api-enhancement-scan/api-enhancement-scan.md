...
description: Given a library or module name, survey every call site of it across the
  ...
  understand how a module is actually used by its callers. Pass --workflow to have it
  propose a reusable Workflow script for the survey instead of running it ad hoc.
argument-hint: "[module-or-library-name] [--workflow]"
allowed-tools:
  - Read
  - Glob
  - Grep
...
**Option:** `--workflow` — instead of running the survey ad hoc, propose a
reusable Workflow script that performs it (see Step 6).
...
2. **Partition the codebase for survey.** Enumerate the call sites — grep the
   module's imports and usages — then group them into independent **sections** by
   package, top-level directory, or subsystem, each small enough that one agent
   can survey it without exhausting its context. Drop sections with no call
   sites.
...
5. **Summarize the total results.** For each surviving cluster, propose one
   ...
   first. End by naming the single highest-leverage enhancement.
